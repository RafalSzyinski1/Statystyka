...
Wykres gęstości pokazuje, gdzie wartości są najbardziej skoncentrowane. Obszar pod krzywą jest równy 1, co oznacza, że wykres gęstości przedstawia rozkład prawdopodobieństwa danej zmiennej.
...
TODO: do poprawy
Prawdopodobieństwo punktowe powinno być równe 0 gdyż wykres pod wykresem w danych punkcie jest równe 0 dla wykresu ciągłego. Jeśli chcemy wyznaczyć prawdopodobieństwo punktowe należy skorzystać z wykresu dyskretnego, które będzie największym przybliżeniem wartości.

```{r probability}
x_point <- 1969 
n <- tail(which(x == x_point),1)

# Prawdopodobieństwo punktowe P(X = 1929)
continuous_dnorm[n] 
discreet_dbinom[n]

# Prawdopodobieństwo przedziałowo P(X <= 1929)
continuous_pnorm[n]
discreet_pbinom[n]
